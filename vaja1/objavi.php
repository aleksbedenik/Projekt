...
function publish($title, $desc, $img, $kategorija){
	...
	$query = "INSERT INTO ads (title, description, user_id, image, datum, datum_zapadlosti, kategorija)
				VALUES('$title', '$desc', '$user_id', '$img_file', CURRENT_DATE, CURRENT_DATE + INTERVAL 30 DAY, '$kategorija');";
	if($conn->query($query)){
		return true;
	}
	else{
